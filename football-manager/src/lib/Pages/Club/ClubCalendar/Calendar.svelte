<script>
  ...
  function getWeekNumber(date) {
    ...
    return Math.ceil((firstDayOfYear.getDay() + 1 + daysSinceFirstDayOfYear) / 7);
  }
  ...
  function generateDaysInMonth(month, year) {
    ...
    const daysBeforeFirstDayOfMonth = firstDayOfMonth.getDay();
    ...
    const daysAfterLastDayOfMonth = 6 - lastDayOfMonth.getDay();
...
let daysInCurrentMonth = generateDaysInMonth(currentDate.getMonth(), currentDate.getFullYear());
...
function handleArrowClick(direction) {
  ...
  daysInCurrentMonth = generateDaysInMonth(currentDate.getMonth(), currentDate.getFullYear());
}
...
<div class="bg-slate-500 dark:bg-slate-800 static justify-evenly flex-col p-4 md:p-12 lg:flex-row gap-2 rounded-xl items-center lg:mx-44 animate-fade-in">
  ...
  <div class="flex items-center">
    ...
    <div class="mx-4 text-2xl">{months[currentDate.getMonth()]} {currentDate.getFullYear()}</div>
    <button class="text-white rounded-full bg-gray-400 hover:bg-gray-600 p-2" on:click={() => handleArrowClick(1)}>
      <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  <div class="grid grid-cols-7 gap-2 mt-4">
    {#each weekdays as day}
      <div class="bg-slate-400 dark:bg-slate-700 py-2 rounded-xl text-white text-center font-bold">{day}</div>
    {/each}
